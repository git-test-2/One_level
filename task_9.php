            ...
            <div class="panel-container show">
                <div class="panel-content">
                    <div class="d-flex flex-wrap demo demo-h-spacing mt-3 mb-3">
                        ...
                        <!-- массив с пользователями, уже не нужен, есть БД -->
                        <!--                               --><?php
                        //                                    $people = [
                        //                                            [
                        ...
                        //сформировали запрос sql
                        $sql = "SELECT * FROM people";
                        //готовлю свой запрос $pdo->prepare($sql); и отправляю в statement
                        $statement = $pdo->prepare($sql);
                        $statement->execute(); //выполняю запрос
                        $people =  $statement->fetchAll(PDO::FETCH_ASSOC); //получаю массив с данными из БД
                        ?>

                        <?php foreach ($people as $user): ?>
                            <!-- тернарный оператор при выводе класса css -->
                            <div class="<?= $user['status'] == 'banned' ? 'banned' : '' ; ?>  rounded-pill bg-white shadow-sm p-2 border-faded mr-3 d-flex flex-row align-items-center justify-content-center flex-shrink-0">
                                ...
                                <div class="ml-2 mr-3">
                                    ...
                                    <a href="<?= $user['twitter_link_href']; ?>" class="text-info fs-sm" target="_blank"><?= $user['twitter_link_text']; ?></a> -
                                    <a href="<?= $user['email_link_href']; ?>" class="text-info fs-sm" target="_blank" title="<?= $user['email_link_title']; ?>"><?= $user['email_link_text']; ?></a>
                                </div>
                            </div>
                        <?php endforeach; ?>
